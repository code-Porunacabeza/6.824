...
func make_config(t *testing.T, n int, unreliable bool, snapshot bool) *config {
	...
	cfg := &config{}
	cfg.t = t
	cfg.net = labrpc.MakeNetwork()       //网络RPC
	cfg.n = n                            //raft集群设备数
	cfg.applyErr = make([]string, cfg.n) //应用层访问用
	cfg.rafts = make([]*Raft, cfg.n)     //每个raft机器配置
	cfg.connected = make([]bool, cfg.n)  //集群中每个机器的网络状态
	...
	cfg.setunreliable(unreliable) //设置机器网络状态

	cfg.net.LongDelays(true) //配置网络

	applier := cfg.applier //应用层接口
	if snapshot {
